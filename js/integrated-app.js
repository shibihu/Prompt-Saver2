...
async function uploadBackupDataToCloud(parsedBackupData) {
  ...
  for (const promptItem of parsedBackupData) {
    try {
      const normalizedItem = { isPinned: false, ...promptItem };
      const updatedPrompts = await savePromptToServer(normalizedItem);
      if (updatedPrompts) {
        prompts = updatedPrompts;
      }
    } catch (singleError) {
      console.error('Error uploading single prompt item:', singleError);
    }
  }
  ...
  reader.onload = function(e) {
    try {
      ...
      if (!Array.isArray(parsedBackupData)) {
        alert('Verification failure: Chosen backup file configuration parameters are corrupt or invalid.');
        return;
      }
      
      if (confirm(`Are you sure you want to load this backup? It will replace your current list of ${prompts.length} prompts.`)) {
        uploadBackupDataToCloud(parsedBackupData);
      }
    } catch (err) {
      alert('Error parsing document content metadata. Ensure file format is valid JSON.');
    }
  };
  reader.readAsText(file);
}
